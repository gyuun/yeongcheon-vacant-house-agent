...
class RedevelopmentReportKind(str, Enum):
    """Specialized report type generated by a redevelopment-use sub-agent."""
...
class BuildingLedgerInfo:
    ...
    behind this model so the redevelopment-use graph can be wired before the
    API adapter is finalized.
    """
...
@dataclass(frozen=True)
class RedevelopmentSubAgentReport:
    """Report produced by a specialized sub-agent in the redevelopment-use workflow."""

    kind: RedevelopmentReportKind = field(metadata={"description": "Report category."})
    summary: str = field(metadata={"description": "Human-readable report summary."})
    context_signals: list[str] = field(
        default_factory=list,
        metadata={"description": "Observed context signals useful for redevelopment-use inference."},
    )
    opportunity_signals: list[str] = field(
        default_factory=list,
        metadata={"description": "Signals that support a specific reuse or redevelopment direction."},
    )
    recommended_actions: list[str] = field(
        default_factory=list,
...
class VacantHouseRecord:
    ...
    land-size context. Latitude/longitude are optional so records can represent
    the current parcel-only source while also supporting coordinate-aware
    nearby-context analysis when coordinates are provided. API-specific fields
    that do not yet belong in the core schema can be stored in `metadata`.
    """
...
@dataclass(frozen=True)
class RedevelopmentRecommendation:
